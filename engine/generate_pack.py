...
import argparse, csv, hashlib, importlib.util, json, math, os, random, re, shutil, struct, sys, time, uuid, zipfile
import base64, gzip
...
try:
    from version import GENERATOR_VERSION, ENGINE_BUILD_ID, EVENT_PROTOCOL_VERSION, CACHE_SCHEMA_VERSION
except ModuleNotFoundError:
    _version_spec = importlib.util.spec_from_file_location(
        'wotlk_engine_version', Path(__file__).resolve().with_name('version.py'))
    ...
    GENERATOR_VERSION = _version_module.GENERATOR_VERSION
    ENGINE_BUILD_ID = _version_module.ENGINE_BUILD_ID
    EVENT_PROTOCOL_VERSION = _version_module.EVENT_PROTOCOL_VERSION
    CACHE_SCHEMA_VERSION = _version_module.CACHE_SCHEMA_VERSION
...
def _resolve_data_source(data_dir, explicit_path, filename):
...
def _output_dir_for_seed(output_root, seed):
    root = Path(output_root).expanduser().resolve()
    return root / f'generated-{seed}'
...
UI_MODES = ('auto','fancy','plain','json')
...
class JsonEventUI(PlainTerminalUI):
    """Newline-delimited JSON event adapter used by the desktop app."""
    def __init__(self,stream=None,**kwargs):
        super().__init__(stream=stream or sys.stdout,quiet=False,animations=False,show_items=True)

    def _emit(self,type_,**payload):
        event={'protocol_version':EVENT_PROTOCOL_VERSION,'type':type_}
    ...
    def startup(self,detail=''):
        self._emit('phase',name='Starting WotLK item forge',detail=detail,total=None)
    ...
    def configure(self,runtime):
        self.runtime=runtime
        fields=(
            'seed','source','output_dir','number','class_name','classes','class_counts',
            'expansion','expansion_level_range','strict_expansion_scoping','generate_sockets',
            'generate_legendaries','magic_effects','socket_bonuses','loot_destinations',
    ...
    def source_check(self,label,path,ok=True):
        self._emit('source_check',label=label,path=path,ok=bool(ok))

    def phase(self,name,total=None,detail=''):
        self._phase=name
        self._emit('phase',name=name,detail=detail,total=total)

    def progress(self,completed,total,current='',class_name=None,class_completed=None,class_total=None):
        self._emit('progress',completed=int(completed),total=int(total or 0),current=current,
                   class_name=class_name,class_completed=class_completed,class_total=class_total)

    def status(self,text):
        self._emit('status',message=text)

    def event(self,kind,title,detail=''):
        self._emit('discovery',kind=kind,title=title,detail=detail)
    ...
    def phase_done(self,name=None,detail=''):
        self._emit('status',message=f"{name or self._phase} complete" + (f" — {detail}" if detail else ''))

    def validation(self,errors,name_changes=()):
        errors=list(errors or ())
        name_changes=list(name_changes or ())
        self._emit('validation',error_count=len(errors),errors=errors,
                   name_repair_count=len(name_changes),name_repairs=name_changes)

    def complete(self,report,elapsed,output_dir):
        self._emit('complete',report=report,elapsed=float(elapsed),output_dir=output_dir)

    def error(self,message,stage=None,details=None):
        self._emit('error',stage=stage,message=str(message),details=details)
...
def create_terminal_ui(args,stream=None,is_tty=None,rich_available=None):
    ...
    if mode=='json':
        return JsonEventUI(stream=stream)
...
def _classes_arg(value):
    try:
        return _interactive_class_names(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(str(exc)) from exc


def _loot_destinations_arg(values):
    selected=set(values or ())
    if 'none' in selected:
        if len(selected) != 1:
            raise argparse.ArgumentTypeError("'none' cannot be combined with other loot destinations")
        return frozenset()
    return frozenset(selected)

def parse_args(argv=None):
    parser=argparse.ArgumentParser(description='Generate randomized AzerothCore WotLK items.')
    parser.add_argument('--output-root',type=Path,default=None,metavar='PATH',help='Directory under which generated-<seed> output folders are created.')
    parser.add_argument('--data-dir',type=Path,default=None,metavar='PATH',help='Directory containing the default SQL and DBC input files.')
    parser.add_argument('--source-cache-file',type=Path,default=None,metavar='PATH',help='Override the compressed source-analysis cache file location.')
    ...
    class_group.add_argument('--class',dest='class_name',type=_class_arg,help='Generate items for only this class (case-insensitive).')
    class_group.add_argument('--classes',dest='class_names',type=_classes_arg,default=None,metavar='LIST',help='Generate items for a comma-separated class list or ALL.')
    ...
    parser.add_argument('--magic-effects',choices=MAGIC_EFFECT_CHOICES,default='All',help='Restrict generated magic effects (default: All).')
    parser.add_argument('--loot-destinations',nargs='+',choices=('world','dungeon','raid','none'),default=None,metavar='DEST',help='Loot insertion destinations: world, dungeon, raid, or none.')
    ...
    parser.add_argument('--no-legendaries',dest='generate_legendaries',action='store_false',default=True,help='Do not promote generated items to Legendary quality.')
    ...
    parser.add_argument('--ui',choices=UI_MODES,default='auto',help='Display mode: auto, fancy, plain, or json (default: auto).')
    ...
    args.disabled_features=sorted(set(args.disabled_features) | set(MAGIC_EFFECT_DISABLED_FEATURES[_normalise_magic_effect(args.magic_effects)]))
    args.loot_destinations=DEFAULT_LOOT_DESTINATIONS if args.loot_destinations is None else _loot_destinations_arg(args.loot_destinations)
    return args
...
def configure_runtime(argv=None,now=None,guid_path=None,args=None,ui=None):
    ...
    if getattr(args,'source_cache_file',None):
        SOURCE_CACHE_FILE=Path(args.source_cache_file).expanduser().resolve()
    FEATURE_CATALOG=None
    ...
    if content_manifest is not None and (args.number is not None or args.class_name is not None or getattr(args,'class_names',None) is not None):
        raise ValueError('--number and class selection cannot be combined with --content-manifest; put counts and classes in the manifest')
    data_dir=Path(args.data_dir).expanduser().resolve() if getattr(args,'data_dir',None) else DATA_DIR.resolve()
    quest_template_source=Path(args.quest_template_source).expanduser().resolve() if args.quest_template_source else None
    world_loot_source=_resolve_data_source(data_dir,args.world_loot_source,'creature_loot_template.sql')
    reference_loot_source=_resolve_data_source(data_dir,args.reference_loot_source,'reference_loot_template.sql')
    item_template_source=_resolve_data_source(data_dir,args.item_template_source,'item_template.sql')
    ...
    gameobject_source_paths=resolve_optional_gameobject_sources(
        requested_gameobject_paths, data_dir=data_dir)
    if args.item_dbc_sources:
        item_dbc_sources=[Path(path).expanduser().resolve() for path in args.item_dbc_sources]
    else:
        item_dbc_sources=[data_dir/'Item.dbc']
        custom_item_dbc=data_dir/'Item.custom.dbc'
        if custom_item_dbc.is_file():
            item_dbc_sources.append(custom_item_dbc)
    item_set_dbc_source=_resolve_data_source(data_dir,args.item_set_dbc_source,'ItemSet.dbc')
    spell_dbc_source=_resolve_data_source(data_dir,args.spell_dbc_source,'Spell.dbc')
    spell_enchantment_dbc_source=_resolve_data_source(data_dir,args.spell_enchantment_dbc_source,'SpellItemEnchantment.dbc')
    disenchant_source=_resolve_data_source(data_dir,args.disenchant_source,'disenchant_loot_template.sql')
    spell_proc_source=_resolve_data_source(data_dir,args.spell_proc_source,'spell_proc.sql')
    spell_script_names_source=_resolve_data_source(data_dir,args.spell_script_names_source,'spell_script_names.sql')
    encounter_paths=(data_dir/'Map.dbc',data_dir/'MapDifficulty.dbc',data_dir/'DungeonMap.dbc',
                     data_dir/'creature.sql',data_dir/'creature_template.sql',data_dir/'instance_encounters.sql',
    ...
    OUT=_output_dir_for_seed(args.output_root or ROOT,SEED)
    SQLDIR=OUT / 'sql'
    return {
...
def _merge_stock_evidence(evidences,source_kind='profile_aggregate',encounter_kind='profile'):
    ...
    if not usable:
        return infer_safe_band((),source_kind,encounter_kind) | {
            ...
            'rejected_required_level_count':sum(e.get('rejected_required_level_count',0) for e in evidences),
            'rejected_required_levels':tuple(sorted({int(level) for e in evidences for level in e.get('rejected_required_levels',())})),
    ...
    band.update({'required_levels':tuple(sorted(required)),'qualities':tuple(qualities),
                 ...
                 'rejected_required_level_count':sum(evidence.get('rejected_required_level_count',0) for evidence in evidences),
                 'rejected_required_levels':tuple(sorted({int(level) for evidence in evidences for level in evidence.get('rejected_required_levels',())})),
                 'reference_provenance':[row for evidence in evidences for row in evidence.get('reference_provenance',())],
...
def _progression_band_width_limit(encounter_kind):
    if encounter_kind in ('profile','aggregate'):
        return 30
...
def infer_safe_band(values,source_kind='profile_aggregate',encounter_kind='boss',prefer_high=False):
    ...
    width=hi-lo
    hard_limit=_progression_band_width_limit(encounter_kind)
    invalid_reason=''
...
def _profile_stock_records(profile,item_cluster):
    evidence=profile.get('evidence') or {}
    records=tuple(evidence.get('_candidate_item_records') or evidence.get('_stock_item_records',()))
    values=set(item_cluster.get('values',()))
    return tuple(row for row in records if int(row.get('item_level',0)) in values)


def _paired_required_cluster(profile,item_cluster):
    records=_profile_stock_records(profile,item_cluster)
    required=[int(row.get('required_level',0)) for row in records
              if int(row.get('required_level',0))>0]
    if not required:
        return None
    prefer_high=bool(((profile.get('evidence') or {}).get('progression_cluster') or {}).get('prefer_high',False))
    clusters=progression_clusters(required,3,prefer_high=prefer_high)
    return max(clusters,key=lambda row:(row['count'],row['values'][0],row['values'][-1])
               if prefer_high else
               (row['count'],-row['values'][0],-row['values'][-1]))
...
def _recompute_evidence_cluster(evidence,cluster,required_cluster=None,encounter_kind='profile'):
    ...
    result.update({'required_levels':required,'qualities':tuple(sorted({int(row.get('quality',0)) for row in selected if int(row.get('quality',0))>0})),
                   ...
                   'rejected_required_level_count':evidence.get('rejected_required_level_count',0),
                   'rejected_required_levels':tuple(sorted({int(level) for level in evidence.get('rejected_required_levels',())})),
...
def _sibling_cluster_support(profile,cluster,sibling):
    if cluster['count']<3: return None
    band=infer_safe_band(cluster['values'],'profile_aggregate',
                         _sibling_profile_band_kind(profile))
    if not band.get('valid'): return None
    candidate_records=_profile_stock_records(profile,cluster)
    if len(candidate_records)<3: return None
    ...
    sibling_records=_profile_stock_records(sibling,active_row)
    if len(sibling_records)<3: return None
    if abs(cluster['center']-active_row['center'])>max(15,cluster['width'],active_row['width']): return None
    sibling_required=_paired_required_cluster(sibling,active_row)
    candidate_required=_paired_required_cluster(profile,cluster)
    if sibling_required is None or candidate_required is None: return None
    if abs(candidate_required['center']-sibling_required['center'])>15: return None
