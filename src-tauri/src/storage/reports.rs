...
impl Database {
    ...
    pub fn list_reports(&self, run_id: &str) -> Result<Vec<ReportRecord>> {
        self.with_conn(|connection| {
            ...
            let rows = statement.query_map([run_id], |row| Ok(ReportRecord { id: row.get(0)?, run_id: row.get(1)?, report_type: row.get(2)?, path: row.get(3)?, import_state: row.get(4)?, size: row.get::<_, i64>(5)? as u64, modified_at: row.get(6)? }))?.collect::<std::result::Result<Vec<_>, _>>()?;
            Ok(rows)
        })
    }
    ...
    pub fn get_report(&self, report_id: i64) -> Result<ReportRecord> {
        self.with_conn(|connection| {
            connection.query_row(
                ...
                [report_id],
                |row| Ok(ReportRecord { id: row.get(0)?, run_id: row.get(1)?, report_type: row.get(2)?, path: row.get(3)?, import_state: row.get(4)?, size: row.get::<_, i64>(5)? as u64, modified_at: row.get(6)? }),
    ...
    pub fn read_report_table(
        ...
        if path.extension().and_then(|value| value.to_str()) == Some("json") {
            ...
            return Ok(ReportTablePage {
                report_type: report_type.into(),
                headers: vec!["Field".into(), "Value".into()],
                rows,
                offset,
                limit,
                has_more: (offset as usize + limit as usize) < total,
                raw_json: Some(raw),
            });
        }
        ...
        Ok(ReportTablePage {
            ...
            offset,
            limit,
            has_more: (offset as usize + limit as usize) < total_seen,
            raw_json: None,
        })
    }
}
...
fn import_placements(
    ...
    for record in reader.records() {
        ...
        let destination = get(&record, &positions, "destination")
...
mod tests {
    ...
    fn indexes_reports_and_imports_world_and_encounter_placements() {
        ...
        let dir =
            std::env::temp_dir().join(format!("wotlk-report-placement-{}", uuid::Uuid::new_v4()));
        ...
        fs::write(
            ...
            "{\"entry\":900001,\"name\":\"Test Blade\"}\n",
        )
        .unwrap();
        db.index_run_pack(&run.id, &dir).unwrap();
        fs::write(
            dir.join("validation_report.json"),
            "{\"validation_errors\":0,\"item_count\":1}",
        )
        .unwrap();
        fs::write(
            dir.join("world_item_placements.csv"),
            "entry,name,destination,pool_id,chance\n900001,Test Blade,world,3000001,2.0\n",
        )
        .unwrap();
        ...
        fs::write(
            dir.join("set_manifest.csv"),
            "set_id,set_name,entry,name\n1,Test Set,900001,Test Blade\n",
        )
        .unwrap();
        ...
        assert!(detail
            .placements
            .iter()
            .any(|placement| placement.destination == "world"));
        assert!(detail
            .placements
            .iter()
            .any(|placement| placement.destination == "raid"));
        let page = db
            .read_report_table(&run.id, "validation", ReportPageRequest::default())
            .unwrap();
    ...
    fn resolves_only_indexed_report_ids() {
        ...
        assert_eq!(
            report.path,
            dir.join("validation_report.json").display().to_string()
        );
        assert!(matches!(
            db.get_report(9_999_999),
