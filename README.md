...
A deterministic, large-scale item generation tool for **AzerothCore / World of Warcraft: Wrath of the Lich King 3.3.5a (build 12340)**.

This generator creates complete batches of custom equipment for the ten playable WotLK classes, writes server-side `item_template` SQL, builds level-appropriate shared world-loot pools, produces GM `.additem` command lists, and creates a merged client-side `Item.dbc` so the generated entries can use existing WotLK models, textures, and inventory icons.

It is designed for large content packs, not just one-off items. The default run creates **100,000 items**, split evenly at **10,000 items per class**.
