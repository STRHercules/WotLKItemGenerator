...
class PlainTerminalUI:
    ...
    def complete(self,report,elapsed,output_dir):
        if self.quiet:
            print(f"Generation complete: {report.get('item_count',report.get('total_items',0)):,} items -> {output_dir}",file=self.stream,flush=True)
            return
...
def write_outputs(items,ui=None):
...
def main(argv=None):
    ...
    try:
        ...
        report=write_outputs(items,ui=ui)
