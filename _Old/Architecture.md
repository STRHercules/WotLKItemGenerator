...
When a manifest profile includes `map_id` and `difficulty_id`, the same source audit verifies map/difficulty identity, creature/gameobject source membership, loot IDs, boss credit entries, and existing creature/reference/gameobject loot targets before writing SQL. Gameobject sources are optional and use the trio rules above. `gameobject_template.Data1` maps a type-3 object to `gameobject_loot_template.Entry`; the object is usable only when that loot entry is present. A static map/spawn row is map-only evidence and remains in `gameobject_reward_targets.csv`; it is never guessed as a boss reward. Rejected static spawn rows are aggregated per map, reward object, and loot entry (`spawn_count`) by default; `--verbose-audit` keeps the full per-spawn rows. Encounter association requires an explicit instance mapping or a `SummonGameObject`/`DoRespawnGameObject` completion path with explicit completion evidence: a boss-death hook such as `JustDied`, an instance encounter state write (`m_auiEncounter[...] = DONE`, `SetBossState(..., DONE)`, `IsBossDone`), a kill-credit spell that `instance_encounters` also credits, or a `DONE` guard condition. Conditional and macro summons (`IsHeroic() ? normal : heroic`, `DUNGEON_MODE(...)`, `RAID_MODE(...)`, local variables bound to reward constants) are resolved per variant and carry a normal/heroic/10/25 difficulty scope. Scripts are anchored to a map through their explicit `InstanceMapScript("...", MAP_X)` registration, or through the same directory's unambiguous instance-script registration; summons without such an anchor stay fail-closed `script_summon` audit rows. The scanner also records `SummonCreature(NPC_*)` relationships as instance-script creature map evidence and reports files scanned, candidate reward calls, candidate creature summons, validated mappings, and rejected mappings with reasons. The script source status is `exercised` only when at least one mapping validated, `exercised_no_reward_mappings` when the scan ran without one, and `not_exercised` when no source root was given. Same-file symbol co-occurrence is insufficient. Gameobject source labels in the audit are portable and never expose host directories.

Shared references are accepted only with verified parent provenance: the consumer, parent target, exact map, and applicable LootMode must agree, and the reference must not be consumed by another map. A difficulty-specific parent must match the profile difficulty exactly; a base parent is accepted only when it is the resolved target's own loot table for that profile. Nested chains are revalidated hop by hop rather than inheriting trust, are bounded by `MAX_REFERENCE_PROVENANCE_DEPTH`, and record `provenance_depth` per edge. Sibling progression coherence requires a real alternate cluster for the same map; `sibling_progression` diagnostics protect the retained band from one-row high-level outliers and report active conflicts as an integration failure. A higher difficulty is promoted to its lower-difficulty sibling's band floor when the profile has source-backed items there, so Heroic can never resolve weaker than its own Normal sibling without a recorded `difficulty_progression_regression` conflict. A sibling difficulty that resolves a disjoint encounter family in a different progression era and cannot be reconciled is excluded from placement with a `sibling progression era conflict` reason on its coverage row instead of masquerading as the modern era. Equivalent safe profiles may differ by up to one item-tier step (`ENCOUNTER_TIER_ITEM_LEVEL_STEP = 13`) in retained cluster width, so same-tier Heroic five-player and raid profiles compete for the same generated item; stock quality evidence remains a soft rendezvous weight and no round is hardcoded to a destination type. Equivalent safe profiles use deterministic BLAKE2b rendezvous distribution, so input order cannot change the selected destination. Distribution concentration, `reference_provenance`, and gameobject association audits are always emitted, including when encounter SQL is gated.

Every creature encounter target must be proven for the profile's map. Accepted evidence is a static creature spawn on that map, an instance-script `SummonCreature` relationship anchored to that map, or an `instance_encounters` map link that is only trusted for a creature with no static spawn on a map without static spawn data. A bare `lastEncounterDungeon` link never overrides a populated map, so classic junk rows such as Mutanus/574 cannot attach a creature to an unrelated dungeon. Script constants are resolved per script directory first and only fall back to a tree-wide value when that token is unambiguous, so the same NPC name in two instances cannot cross-resolve. `instance_script_summon` evidence never relocates a creature already proven by a static spawn, conflicting script maps are only trusted when the encounter credit agrees, and ambiguous script evidence is dropped with a `cross_map_evidence_conflicts` audit entry. `validate_encounter_integration` fails closed when a target has no proven map evidence, when its declared difficulty resolution contradicts the profile difficulty, or when the same encounter id/source creature/effective creature is reused across unrelated maps.
...
The generated report records resolved encounter order, rank, ItemLevel and RequiredLevel bands, source evidence, quality limits, weights, chances, quantities, set decisions, coverage exclusions, difficulty comparisons, rejected references, sibling coherence/outlier and progression-regression diagnostics, deterministic distribution audits, reference provenance with per-edge depth, aggregated gameobject spawn rejections, gameobject reward-target status, `script_reward_mapping` status, and encounter integration validity. A Heroic coverage gap fails closed only when its evidence is actionable; maps without static target evidence whose rewards depend on unresolved scripted caches are reported as warnings.
